...
### Phase 0: インフラ・基盤（Windsurf + Opus 4.6 で実装）

| #   | タイトル                                                              |
| --- | --------------------------------------------------------------------- |
| -   | 環境構築（Docker Compose, Next.js 16, PostgreSQL, Prisma, shadcn/ui） |
| -   | globals.css カラー定義（OKLCH + @theme inline）                       |
| -   | Prisma スキーマ + seed データ                                         |
| -   | Auth.js v5 + 認証画面（ログイン / サインアップ / proxy.ts）           |
| -   | レイアウト（サイドバー + ヘッダー + ThemeProvider）                   |
| -   | タスク一覧 + 作成（お手本1機能）                                      |
| -   | OpenAPI 仕様ドラフト                                                  |

### Phase 1: 基盤・ドキュメント（Issue #1〜#3）

| #   | タイトル                  | 対応US              | 優先度 |
| --- | ------------------------- | ------------------- | ------ |
| 1   | ERD図の作成（Mermaid）    | -                   | High   |
| 2   | OpenAPI 仕様の完成        | -                   | High   |
| 3   | プロジェクトCRUD API + UI | US-04, US-05, US-06 | High   |

### Phase 2: コア機能（Issue #4〜#13）

| #   | タイトル                                | 対応US       | 優先度 |
| --- | --------------------------------------- | ------------ | ------ |
| 4   | プロジェクトメンバー管理                | US-07, US-08 | High   |
| 5   | タスク詳細画面                          | US-10        | High   |
| 6   | タスクのインライン編集                  | US-10        | High   |
| 7   | タスク削除                              | US-11        | Medium |
| 8   | サブタスク機能                          | US-12        | Medium |
| 9   | カンバンボード D&D                      | US-14        | High   |
| 10  | カンバン クイック作成                   | US-15        | Medium |
| 11  | タスク一覧（リストビュー）完成          | US-16        | High   |
| 12  | カテゴリ/ラベル管理                     | US-20        | Medium |
| 13  | タスクへのカテゴリ付与 + フィルタリング | US-20        | Medium |

### Phase 3: 拡張機能（Issue #14〜#21）

| #   | タイトル                               | 対応US | 優先度 |
| --- | -------------------------------------- | ------ | ------ |
| 14  | コメント機能                           | US-19  | High   |
| 15  | コメント @メンション                   | US-19  | Medium |
| 16  | ファイル添付機能                       | US-21  | Medium |
| 17  | グローバル検索                         | US-22  | High   |
| 18  | 検索フィルター                         | US-22  | Medium |
| 19  | アクティビティログ（タスク）           | US-23  | Medium |
| 20  | アクティビティフィード（プロジェクト） | US-24  | Medium |
| 21  | ダッシュボード（サマリー + グラフ）    | US-18  | High   |

### Phase 4: UX・運用機能（Issue #22〜#27）

| #   | タイトル                | 対応US | 優先度 |
| --- | ----------------------- | ------ | ------ |
| 22  | ガントチャート          | US-17  | Medium |
| 23  | アプリ内通知            | US-25  | High   |
| 24  | メール通知（Resend）    | US-26  | Medium |
| 25  | 多言語対応（next-intl） | US-27  | Medium |
| 26  | ダークモード            | US-28  | Medium |
| 27  | 監査ログ（管理者画面）  | US-29  | Low    |

---

## Issue テンプレート

### 例: Issue #1 ERD図の作成（Mermaid）
...
- ユーザーストーリー: docs/user-stories.md US-14
...
| ------------------ | ------------------ | ------------------- | --------------------- | -------------- |
| F01 認証           | US-01〜03          | #1〜3               | auth/\*               | Phase 0        |
| F02 プロジェクト   | US-04〜08          | #5〜6, #11          | projects/\*           | #3, #4         |
| F03 タスクCRUD     | US-09〜12          | #7〜8, #10          | tasks/\*              | Phase 0, #5〜8 |
| F04 カンバン       | US-13〜15          | #7                  | tasks/\*/status, sort | #9, #10        |
| F05 リスト         | US-16              | #8                  | projects/\*/tasks     | #11            |
| F06 ガント         | US-17              | #9                  | tasks/\*              | #22            |
| F07 ダッシュボード | US-18              | #4                  | dashboard             | #21            |
| F08 コメント       | US-19              | #10                 | comments/\*           | #14, #15       |
| F09 カテゴリ       | US-20              | #11                 | categories/\*         | #12, #13       |
| F10 添付           | US-21              | #10                 | attachments/\*        | #16            |
| F11 検索           | US-22              | 全画面ヘッダー      | search                | #17, #18       |
| F12 アクティビティ | US-23〜24          | #10, ダッシュボード | activities            | #19, #20       |
| F13 通知           | US-25〜26          | #13                 | notifications/\*      | #23, #24       |
| F14 i18n           | US-27              | 全画面              | users/me              | #25            |
| F15 ダークモード   | US-28              | 全画面              | users/me              | #26            |
| F16 監査ログ       | US-29              | #15                 | admin/audit-logs      | #27            |
| F17 ユーザー       | US-08, US-30       | #12, #14            | users/me, members/\*  | #4             |
